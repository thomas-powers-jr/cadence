---
cadence_handoff: 1
generated_at: 2026-09-18T16:40:31.874Z
label: 
loop_position: BUILD
...
- **This handoff doc's own PR (#525) is open, held by explicit operator choice** ("hold off") — not yet merged as of this writing. If it's still open when you read this, the operator wanted to review it first; check its state before assuming it landed.
...
**PR #525 — this handoff.** Opened, CI green, held open at the operator's explicit request ("hold off") rather than self-merged. Regenerated once already (`cadence handoff` overwrote the skeleton back to blank when re-run on `main`, since `main` hadn't advanced past d972b0af yet) — the narrative below was restored from the open PR branch rather than retyped from scratch.
...
- **Re-running `cadence handoff` on a branch that hasn't advanced past the point of a prior (unmerged) handoff overwrites the same-dated file back to a blank skeleton** — it's a fresh scaffold each time, not an update. If a prior handoff PR for the same date is still open, pull its content back (`git show <branch>:<path>`) rather than retyping.
...
No mandatory action — loop is IDLE, PRs #522/#523/#524 are merged and independently verified (not just trusted from a workflow report); #525 (this doc) is open awaiting the operator's own review/merge. Candidates, in rough priority:
1. Merge or review PR #525 (this handoff) at your convenience.
2. Triage `rec-20260918-005/006/007` (phase-310 CRLF/BOM follow-ups) — all `ready-for-cadence-spec`, low-risk, well-scoped.
3. If the phase-314 divergence notice fires in a future worktree-isolated `phase-build`, that's the empirical answer to this session's still-open "does Claude Code's hook subprocess cwd follow a worktree" question — worth a quick follow-up rec either way.
4. Housekeeping batch (deferred three sessions running now): regenerate+commit `RECOMMEND.md`/`recommend.json`; decide the fate of the stale/deleted handoff docs.
5. Otherwise, pull the next candidate from `cadence recommend`.

**Verify:** `git log --oneline -3` should show `d972b0af fix: hook shim subprocess spawns pass through the payload's cwd (phase 314) (#524)` as HEAD on `main` (or later, if #525 or something else has landed since). `gh pr view 522/523/524 --json state` should all read `MERGED`; `gh pr view 525 --json state` may still read `OPEN`.

**If it fails:** N/A — nothing left mid-flight; PRs #522-524 independently confirmed merged, not just trusted from their own CI report.
