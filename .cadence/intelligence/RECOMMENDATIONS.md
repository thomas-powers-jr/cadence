...
## rec-20260918-002 — Superpowers scorecard: systematic-debugging is unclaimed, and the cadence assumption hypothesis ledger has zero callers anywhere in .claude/
...
- confidence: 70%
- decay: fresh
- areas: skills, intelligence
- files: .claude/skills/systematic-debugging/SKILL.md, docs/packs-design.md
- evidence: Zero matches for the assumption CLI across .claude/ (measured). Assumption status enum open|validated|rejected at packages/types/src/intelligence.ts:129; transitions validate|reject|reopen at packages/core/src/cli/commands/assumption.ts:277. assumption list supports --filter-rec, --filter-status and --format json, so the gate query is deterministic. Superpowers systematic-debugging SKILL.md read at superpowers/6.3.0 — four phases, advice only, no ledger. Base main 18d2bf78.
- next: cadence milestone propose

cadence assumption add/show/list/validate/reject/reopen is a full hypothesis ledger with open|validated|rejected states, and nothing in .claude/ has ever used it — measured zero references. Meanwhile the Superpowers scorecard lists systematic-debugging as unclaimed, and that skill is four phases of prompt discipline with no durable trail: reproduce, hypothesize, test, confirm. Shipping a CADENCE debugging skill gives the existing ledger its first caller and makes the trail inspectable after the session: each hypothesis is an assumption row tied to an anchor recommendation, closed validated or rejected, and the conclusion step is gated on a deterministic query (assumption list --filter-rec ID --filter-status open --format json) that must come back empty. Scoped honestly: that gate is a query the skill honors, not an engine-enforced gate — CADENCE cannot refuse a conclusion the way settle refuses a phase, so the deliverable is a discipline with a real substrate underneath it, not enforcement. The skill ships as a file under .claude/skills/ and is referenced from the pack docs, because PackManifestZ has no slot for a distributed-but-not-required skill (rec-20260917-003).
